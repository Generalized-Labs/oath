...
#[derive(Parser)]
#[command(name = "ward", version, about = "Secure package management for the JavaScript ecosystem")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
    ...
    match cli.command {
        Commands::Install { packages, dev } => {
            if packages.is_empty() {
                println!("ward: resolving dependencies...");
            } else {
                let target = if dev { "dev" } else { "prod" };
                println!("ward: installing {} ({target})", packages.join(", "));
            }
            // TODO: wire up ward-resolve + ward-fetch + ward-store
        }
        Commands::Exec { package, args, allow_net, allow_read, allow_write, allow_env } => {
            println!("wardx: executing {package} in sandbox");
            println!("  permissions:");
            ...
            // TODO: wire up ward-sandbox
        }
        Commands::Audit { production, format: _ } => {
            let scope = if production { "production" } else { "all" };
            println!("ward audit: scanning {scope} dependencies...");
            // TODO: wire up ward-analyze
        }
        Commands::Verify { package, all } => {
            if all {
                println!("ward verify: checking all lockfile entries against transparency log...");
            } else if let Some(pkg) = package {
                println!("ward verify: checking {pkg} against transparency log...");
            }
            // TODO: wire up ward-index
        }
        Commands::Perms { package } => {
            println!("ward perms: capabilities declared by {package}:");
            println!("  (not yet implemented)");
            // TODO: read manifest.permissions
        }
        _ => {
            println!("ward: command not yet implemented");
        }
    }
