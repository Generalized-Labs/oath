...
#[derive(Parser)]
#[command(name = "oathx", version, about = "Execute packages in a secure sandbox")]
struct Cli {
    /// Package to execute
    package: String,
...
async fn main() -> anyhow::Result<()> {
    ...
    if cli.allow_all {
        eprintln!("oathx: WARNING -- running with all permissions (no sandbox)");
    }

    println!("oathx: resolving {}...", cli.package);
    println!("oathx: sandbox active");
